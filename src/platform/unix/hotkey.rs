...
impl UnixHotkeyManager {
    ...
    fn try_x11_hotkey(&mut self, callback: Box<dyn Fn() + Send + Sync>) -> Result<()> {
        ...
        let (conn, screen_num) = x11rb::connect(None).map_err(|e| {
            anyhow!(
                "Failed to connect to X11 display: {}. Ensure DISPLAY is set.",
                e
            )
        })?;
        ...
        tracing::debug!(
            "Attempting to grab X11 hotkey: {} (keycode: {})",
        ...
        tracing::info!(
            "Successfully registered X11 global hotkey: {}",
            self.config.combination
        );
        Ok(())
    }
    ...
    fn get_keycode_for_key_name(
        ...
        let keyboard_mapping =
            get_keyboard_mapping(conn, min_keycode, max_keycode - min_keycode + 1)?.reply()?;
                ...
                if let Ok(setup) = std::panic::catch_unwind(|| conn.setup()) {
                    if let Some(screen) = setup.roots.first() {
                        let (modifiers, key_name) =
                            self.parse_hotkey_combination(&self.config.combination)?;
