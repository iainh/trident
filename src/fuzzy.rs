...
impl SearchEngine {
    ...
    pub fn search(&self, query: &str, case_sensitive: bool, max_results: usize) -> Vec<&HostEntry> {
        ...
        let mut results: Vec<(&HostEntry, usize)> = self.hosts
            .iter()
            .filter_map(|host| {
...
fn calculate_fuzzy_score(target: &str, query: &str, case_sensitive: bool) -> usize {
    ...
    if target_normalized.starts_with(&query_normalized) {
        ...
        if target_normalized.len() > query_normalized.len() {
            let next_char = target_normalized.chars().nth(query_normalized.len()).unwrap();
...
mod tests {
    ...
    fn test_fuzzy_match() {
        let hosts = vec![
            HostEntry::new("development-server".to_string(), "ssh development-server".to_string()),
    ...
    fn test_performance_under_50ms() {
        ...
        assert!(duration.as_millis() < 50, "Search took {:?}, should be under 50ms", duration);
    }
    ...
    fn test_fuzzy_scoring_order() {
        let hosts = vec![
            ...
            HostEntry::new("gitlab.company.com".to_string(), "ssh gitlab.company.com".to_string()),
