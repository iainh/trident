...
impl AppState {
    ...
    fn load_hosts(&mut self) -> Result<Vec<HostEntry>> {
        ...
        if self.config.parsing.parse_known_hosts {
            ...
            if known_hosts_path.exists() {
                let hosts = parse_known_hosts(known_hosts_path, self.config.parsing.skip_hashed_hosts)?;
        ...
        if self.config.parsing.parse_ssh_config {
            ...
            if config_path.exists() {
                let hosts = parse_ssh_config(config_path, self.config.parsing.simple_config_parsing)?;
                all_hosts.extend(hosts);
            }
        }
    ...
    fn update_filtered_hosts(&mut self) {
        ...
        if self.selected_index >= self.filtered_hosts.len() {
            self.selected_index = if self.filtered_hosts.is_empty() { 0 } else { self.filtered_hosts.len() - 1 };
        }
    }
...
mod tests {
    use super::*;
    use crate::config::{TerminalConfig, SshConfig, ParsingConfig, UiConfig};
    use std::fs;
    use std::io::Write;
    ...
    fn create_test_config(temp_dir: &TempDir) -> Config {
        ...
        writeln!(known_hosts_file, "example.com ssh-rsa AAAAB3NzaC1yc2EAAAABIwAAAQEA...").unwrap();
        writeln!(known_hosts_file, "server.local ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAI...").unwrap();
    ...
    fn test_update_search_query() {
        ...
        app.update(Message::UpdateSearchQuery("prod".to_string())).unwrap();
