...
mod tests {
    ...
    fn test_escape_shell_command_with_dollar_and_backticks() {
        ...
        let escaped = escape_shell_command(command);
        assert_eq!(escaped, "ssh user@server.com -t \\'echo \\$HOME \\&\\& \\`whoami\\`\\'");
    }
    ...
    fn test_launcher_substitutes_ssh_command() {
        ...
        assert_eq!(substituted, "tell app \"Terminal\" to do script \"ssh user@test-server.com\"");
    }
    ...
    fn test_escape_comprehensive_special_chars() {
        let dangerous_command = "ssh user@server && rm -rf / | echo \"gotcha\" > /tmp/evil; $(whoami)";
